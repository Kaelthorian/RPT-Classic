...
RPT Classic is being migrated incrementally to the Godot 4 project at the parent
repository root. This BYOND tree remains the behavioral reference and must not
be removed, relocated, or rewritten during migration work.

Before porting a system, read its active DME implementation, document behavior
and dependencies, design a small Godot boundary, implement one vertical slice,
verify it against BYOND evidence, and update `../docs/migration/`. Do not perform
bulk `.dm` to `.gd` translation or inspect real runtime data for fixtures.
