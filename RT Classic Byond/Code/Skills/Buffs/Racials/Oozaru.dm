mob/proc/Tail_Add()
	...
	for(var/BodyPart/Tail/O in src)
		var/image/_overlay = image('TailBrown.dmi') // not sure if the equipped thing is an icon/object so
...
mob/proc/Oozaru_Revert(reset=0) if(Oozaru) for(var/BodyPart/Tail/O in src)
	...
	overlays-=HealthBar
	overlays-=EnergyBar
	HealthBar.pixel_x-=abs(O.OozX)
	EnergyBar.pixel_x-=abs(O.OozX)
	overlays-=HealthBar
	overlays-=EnergyBar
	Oozaru=0
	BuffNumber-=3
	...
	//setCooldown("Oozaru",1)





mob/proc/Oozaru() if(!afk)  if(!Oozaru&&!ssj) for(var/BodyPart/Tail/O in src) if(O.Status=="Healthy") if(O.Setting)
...
obj/items/Mark_Of_The_Beast
	desc="Possessing this will cause you to transform into a Werewolf in the full moon."
	icon='enchantmenticons.dmi'
	icon_state="RoR"

obj/items/Ring_Of_Power
	desc="Possessing this will increase your BP by +5%."
	icon='enchantmenticons.dmi'
	icon_state="RoS"

obj/items/Necklace_Of_Wisdom
	desc="Possessing this will cause you to earn 0.25 MP once a year."
	icon='enchantmenticons.dmi'
	icon_state="NoM"

obj/items/Ring_Of_Experience
	desc="Possessing this will cause you to earn +75 EXP once a year."
	icon='enchantmenticons.dmi'
	icon_state="RoA"
...
mob/proc/Golden() if(!afk) if(!Oozaru) for(var/BodyPart/Tail/O in src) if(O.Status=="Healthy")
	...
	if(OozaruMastery>100) OozaruMastery=100
	Oozaru=1
	src.setCooldown("Oozaru",70000)//2
	oicon=icon
	Overlays.Add(overlays)
	overlays.Remove(overlays)
