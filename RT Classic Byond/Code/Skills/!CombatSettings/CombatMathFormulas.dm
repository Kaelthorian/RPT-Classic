proc/CDCalculation(Skill/S,mob/M)
	...
	CDCalculation=((1+Modifier(M))/S.CDT)*(SkillTraitCooldownReduce(S))
	DebugLogs("-CDCalculation-","Combat")
	DebugLogs("Offender: [M]","Combat")
	DebugLogs("Skill: [S]","Combat")
	DebugLogs("SkillTraitCooldownReduce: ((1+Modifier [Modifier(M)])/CDT [S.CDT])*SkillTraitCooldownReduce [SkillTraitCooldownReduce(S)]","Combat")
...
proc/AutoAttackDamageCalculator(mob/Offender,mob/Defender)
	...
	if(Offender&&Defender)
		...
		DebugLogs("Dam Calc=(([Offender.BP]+([Sword_Damage]))*([Offender.Str])","Combat")
		var/Dam=((Offender.BP+(Sword_Damage))*(Offender.Str))*2
		DebugLogs("<font color=yellow>Dam=[Dam]","Combat")
		var/Res=((Defender.BP+(EMPBP))*(Defender.End-(Defender.End*(IgnoresEnd/100))))
		DebugLogs("Res Calc=(([Defender.BP]+([EMPBP]))*([Defender.End]-([Defender.End]*([IgnoresEnd]/100))))","Combat")
...
proc/DamageCalculator(mob/Offender,mob/Defender,Skill/S)
	if(Offender&&Defender)
		...
		DebugLogs("Dam Calc=(([Offender.BP]+([Sword_Damage]*[S.WEAPON]))*(([Offender.Str]*[S.DMGSOURCE_STR])+([Offender.Spd]*[S.DMGSOURCE_SPD])+([Offender.Pow]*[S.DMGSOURCE_POW])+([Offender.Off]*[S.DMGSOURCE_OFF])+([Offender.Def]*[S.DMGSOURCE_DEF])+([Offender.End]*[S.DMGSOURCE_END])))","Combat")
		var/Dam=((Offender.BP+(Sword_Damage*S.WEAPON))*((Offender.Str*S.DMGSOURCE_STR)+(Offender.Spd*S.DMGSOURCE_SPD)+(Offender.Pow*S.DMGSOURCE_POW)+(Offender.Off*S.DMGSOURCE_OFF)+(Offender.Def*S.DMGSOURCE_DEF)+(Offender.End*S.DMGSOURCE_END)))
		DebugLogs("<font color=yellow>Dam=[Dam]","Combat")
		var/Res=((Defender.BP+(EMPBP))*(Defender.End-(Defender.End*(IgnoresEnd/100))))
		DebugLogs("Res Calc=(([Defender.BP]+([EMPBP]))*([Defender.End]-([Defender.End]*([IgnoresEnd]/100))))","Combat")
		DebugLogs("<font color=yellow>Res=[Res]","Combat")
...
proc/DamageCalculatorKiWeapon(mob/Offender,mob/Defender)
	if(Offender&&Defender)
		...
		var/Dam=((Offender.BP+(Sword_Damage*WEAPON))*((Offender.Str*DMGSOURCE_STR)+(Offender.Pow*DMGSOURCE_POW)+(Offender.Spd*DMGSOURCE_SPD)))
		...
		DebugLogs("<font color=yellow>Dam=[Dam]","Combat")
		DebugLogs("Dam= (([Offender.BP]+([Sword_Damage]*[WEAPON]))*(([Offender.Str]*[DMGSOURCE_STR])+([Offender.Pow]*[DMGSOURCE_POW])+([Offender.Spd]*[DMGSOURCE_SPD])))","Combat")
		DebugLogs("Def= (([Defender.BP]+([EMPBP]))*([Defender.End]-([Defender.End]*([IgnoresEnd]/100))))","Combat")
		DebugLogs("<font color=yellow>Res=[Res]","Combat")
		DebugLogs("<font color=red>TotalDamage=[TotalDamage]","Combat")
		DebugLogs("Defender HP=[Defender.Health]","Combat")
		DebugLogs("----------------------------------------","Combat")
...
proc/
	...
	UnarmedEnergyCalculation(mob/Attacker,mob/Defender)
		...
		if(Attacker.ChakraBlocked)
			return 0
		UNARMEDENERGY=(((Attacker.MaxKi*DEFAULT_UNARMED_ENERGY)*Attacker.Str*Attacker.BP)/(Defender.End*Defender.BP)*(1 -(0.1*Attacker.Shielding)))+(Attacker.HasDeepBreathing*2)
	...
	SwordEnergyCalculation(mob/Attacker,mob/Defender)
		...
		if(Attacker.ChakraBlocked)
			return 0
		SWORDENERGY=(((Attacker.MaxKi*DEFAULT_SWORD_ENERGY)*Attacker.Str*Attacker.BP)/(Defender.End*Defender.BP)*(1 -(0.1*Attacker.Shielding)))+(Attacker.HasDeepBreathing*2)
	...
	HammerEnergyCalculation(mob/Attacker,mob/Defender)
		...
		if(Attacker.ChakraBlocked)
			return 0
		HAMMERENERGY=(((Attacker.MaxKi*DEFAULT_HAMMER_ENERGY)*Attacker.Str*Attacker.BP)/(Defender.End*Defender.BP)*(1 -(0.1*Attacker.Shielding)))+(Attacker.HasDeepBreathing*2)
...
proc/SkillAccuracy(mob/Offender,mob/Defender,Skill/S)
	if(Offender&&Defender)
		var/Offense=(Offender.BP*((Offender.Off)+(Offender.Spd*(0.3+(max(Offender.HasPrecognition,Offender.Precognition)*0.1)))))
		var/Defense=(Defender.BP*((Defender.Def)+(Defender.Spd*(0.3+(max(Defender.HasPrecognition,Defender.Precognition)*0.1)))))
...
//		DebugLogs("Energy Drain: [S.EDT]","Combat")
		DebugLogs("Offense:(Offender.BP [Offender.BP]*((Offender.Off [Offender.Off])+(Offender.Spd [Offender.Spd]*(0.3+(Offender.Precognition [Offender.Precognition]*0.05)))))","Combat")
		DebugLogs("Defense:(Defender.BP [Defender.BP]*((Defender.Def [Defender.Def])+(Defender.Spd [Defender.Spd]*(0.3+(Defender.Precognition [Defender.Precognition]*0.05)))))","Combat")
...
proc/KiAccuracyFormula(obj/ranged/Blast/BB,mob/Offender,mob/Defender,Skill/S)
	if(Offender&&Defender)
		...
		DebugLogs("Offender : [Offender] Vs Defender : [Defender]","Combat")
		DebugLogs("<font color=white>Skill: [S]","Combat")
		var/Offense=(Offender.BP*((Offender.Off)+(Offender.Spd*(0.3+(Offender.Precognition*0.05)))))
		var/Defense=(Defender.BP*((Defender.Def)+(Defender.Spd*(0.3+(Defender.Precognition*0.05))))) //changed from 0.15 to .25 by trashman
		DebugLogs("Offense:(Offender.BP [Offender.BP]*((Offender.Off [Offender.Off])+(Offender.Spd [Offender.Spd]*(0.3+(Offender.Precognition [Offender.Precognition]*0.05)))))","Combat")
		DebugLogs("Defense:(Defender.BP [Defender.BP]*((Defender.Def [Defender.Def])+(Defender.Spd [Defender.Spd]*(0.3+(Defender.Precognition [Defender.Precognition]*0.05)))))","Combat")
		DebugLogs("Offense: [Offense]","Combat")
		DebugLogs("Defense: [Defense]","Combat")
