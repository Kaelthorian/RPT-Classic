		...
		if(rows&&cols) // If we've got the rows and columns
			for(rows=rows, rows>0, rows--) // Loop through all the rows
				for(var/i=cols, i>0, i--) // For each row, empty it, then move a column to the side
					winset(src.client,grid,"current-cell=[i],[rows]")
					src<<output(null,grid)

	winset(src.client, grid, "cells=1x1")
...
mob/proc/buildMainCat(gridID)
	...
	if(src)
		...
		if(src)
			...
			winset(src.client, BUILD_CATS, "cells=[cols]x[rows]")

		//winset(src.client, BUILDWINDOW, "on-close=\"closebuild\"")
...
mob/proc/buildSubCats(gridID)
	...
	winset(src.client, BUILD_SUBS, "cells=0x0")
...
mob/proc/updateBuildGrid(categ,expand=0,col=1)
		...
		switch(categ) // See which category it's placing
			...
			if("Roofs")
				...
				for(var/obj/O in build_categ)
					...
					winset(src.client, BUILD_CATS, "current-cell=[col],[row]")
					if(expand!=0)
						O.icon_state = "Minus"
					else
						O.icon_state = "Plus"
				...
				if(expand!=0)
					...
					winset(src.client, BUILD_CONTENT, "cells=0x0")
					...
					//winset(src.client, BUILD_CATS, "current-cell=1,[row]")
					...
					if(2) // Load the objects belong to a subcategory
						...
						for(var/i in buildter_sub)
							for(var/obj/O in buildter_sub)
								...
								winset(src.client, BUILD_SUBS, "current-cell=[col],[row]")
								if(buildter_sub[i]!=0)
									O.icon_state = "Minus"
								else
									O.icon_state = "Plus"
	...
	*/
						...
						//row += 2
						//winset(src.client, "grid_buildCat", "current-cell=1,[row]")
