obj/items/Power_Armor
	...
	New()
		desc="<br>This is a suit of power armor. Entering it will confer [ForceModel?"+20% Force" : "+20% Strength"] and +30% Endurance but reduce your speed by -20%.\
		...
		The suit can be grabbed and moved like a prop. Requires an INT mod of 3 or the New Type milestone to use. It will remove the BP penalty associated with INT milestones. [Commas(Health)] Health Power Armor"
...
//		A.Value-=Cost
	...
	verb/Upgrade()
		...
		desc="<br>This is a suit of power armor. Entering it will confer [ForceModel?"+20% Force" : "+20% Strength"] and +30% Endurance but reduce your speed by -20%.\
		Once the armor is reduced to 0 durability, it will break and eject its user. To exit or enter the armor, simply use the associated verb.  \
		The suit can be grabbed and moved like a prop. Requires the New Type milestone or an int mod of 3+ to enter and use. This will remove the BP loss associated with taking Intelligence milestones. [Commas(A.Health)] Health Power Armor"
	...
	verb/Use_Power_Armor()
		...
		if(src.suffix == "*Equipped*")if(usr.KOd==0)
			usr.Eject(src)
			return
		if(usr.AndroidLevel && usr.Race!="Android")
		...
		if(usr.ArmorOn)
			...
			return
		if(usr.Int_Mod<3&&!usr.HasNewType)
			usr << "You need an intelligence intelligence mod of at least 3 or the New Type milestone to understand how to operate the complex controls used in this suit of power armor!"
			return
		...
		if(src in range(1,usr)) if(src.suffix==null) if(src.Health > 0) if(usr.Int_Level >= 65) if(usr.KOd==0) if(p == src.Password)
			...
			//	if(usr.Int_Mod>=3)
				if (Gundam)
					...
					//	if(usr.HasNewType) usr.BPMult *= 1.325
					//	else usr.BPMult *=1.3

				//	else
					//	if(usr.HasNewType) usr.BPMult *=1.225
					//	else usr.BPMult *= 1.2 */
				if(ForceModel) usr.PowMult *= 1.2
				else usr.StrMult *= 1.2
				if(usr.HasArmorMastery) usr.EndMult*=1.37
				else usr.EndMult*=1.3
				if(usr.HasNewType) usr.SpdMult *= 0.9
				else usr.SpdMult *=0.8
				usr.BreathInSpace += 1
				usr.PowerArmorOn=1
				return
			else
