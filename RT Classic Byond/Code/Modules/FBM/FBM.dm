...
//Racial Transformations
	...
	if(Race=="Namekian")
		if(WipeDay>=21&&!SNj&&!locate(/Skill/Buff/Super_Namekian) in src)
...
/*	if("Post Human" in LearnList)
		if(Race=="Human"&&Base/BPMod>Tier4Req&&Zanzoken>1000&&!locate(/Skill/Buff/PostHumanism) in src)
			contents+=new/Skill/Buff/PostHumanism
			//PostHumanGot=1
			src<<"You have awoken something.  Deep within your mind, a door has been opened and your psyche has ushered in a new power. While neither 'good' nor 'evil' this new force is entirely unfamiliar and foreign and yet, it welcomes you like a new home. You find yourself able to tap into this power, but you are not sure of the cost..."
			alertAdmins("[src] has unlocked Post Humanism")*/
	if(Race=="Android")
		if(WipeDay>=21&&!locate(/Skill/Buff/Overdrive) in src)
			contents+=new/Skill/Buff/Overdrive
			src<<"You got Overdrive."
	if(Race=="Changeling") if(WipeDay>=21&&HasForm==0) // Mumit added BPMod division
		src<<"You have awoken a dormant power deep within yourself."
	...
	if(Global_SSJ2)
		if(Race=="Saiyan"||Race=="Half-Saiyan"||Race=="Part-Saiyan")
			if(HasSSj<2&&SSjDrain>=300&&WipeDay>31)
				src<<"You have managed to achieve the next level of Super Saiyan."
				HasSSj=2
		if(Race=="Heran"||Race=="Half-Heran") if(HasBojack<2&&SSjDrain>=300&&WipeDay>31)
			HasBojack=2
