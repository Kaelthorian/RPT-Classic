...
mob/proc/Death(var/Z,forced=0, MajAbsorber,mob/Killer)
	...
	if(Z!="admin"&&Z!="Admin"&&Z!="Self Destruct"&&!forced) if(EventCharacter==0) if(SaveAge<3) return
	...
	Cyborg=0
	SeanceYear=Year+5
	EraDeathScore(src,Killer)
	for(var/mob/player/A)
		if(A.z==11)
			A<<"<font color=red>The soul of a recent death pierce the veil of the afterlife."
