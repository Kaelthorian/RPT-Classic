...
//			UpdateStats("All")
			...
			oicon=src.icon
			Ki = MaxKi
			BaseMaxKi=KiMod*100				//ENERGYUPDATE
			PlanetGravity()
...
//			if(Confirm("Would you like to start with your stats 50% to the cap? (There is no downside to doing so)")) CapStats(0.5)
			if(src.client.holder && src.client.holder.level >= 5 || (AllowRares&&AllowRares.Find(src.ckey)) || key=="Chewyyyy") //EC HERE KAEL
				if(Confirm("Make an event character?"))
					MakeEC()
			src.Check_Incarnates()
			GetLanguage()
			AlienMPGet() // if(Race=="Alien") spawn() AlienMPGet() --- Was just for CA MPs, now for all. Not changing the proc name though.
			spawn(5)
	...
	donec()
		...
		usr.BaseMaxKi=100								//ENERGYUPDATE REDOSTATS
		usr.BaseMaxKi=usr.KiMod*100
	...
	racec()
		...
		winset(client,"learnD","text=\"Learn List: [GeneticLearnList.Join(", ")]\"")
		CanPing=1
		SelectingRace = 0
...
//			else if(C=="Spirit Doll") Doll()
				...
				if(src) if(client)
					if(src.client && src.client.holder && src.client.holder.level>=3 || AllowRares.Find(ckey) || global.TestServerOn) Changie_Choices+="Frieza"
			...
			if(C.Race=="Demon Contract")
				...
				Builder=C.Builder
				AntiPowerGameKey=C.AntiPowerGameKey
				SelectedAge=1
				Bio()
				Builder=C.Builder
				for(var/mob/M in Players) if(M.key==C.ParentKey||M.key==C.Parent2Key)
...
mob/proc/Location()
	if(global.TestServerOn) loc=locate(25,25,1)
	else
		if(Dead&&SeanceYear<Year) loc=locate(DEADX,DEADY,DEADZ)
			...
			switch(Race)
				...
				if("Kanassa") loc=locate(GetSpawnX(KanassaSpawn),GetSpawnY(KanassaSpawn),GetSpawnZ(KanassaSpawn))
				...
				if("Kaio") loc=locate(GetSpawnX(KaioSpawn),GetSpawnY(KaioSpawn),GetSpawnZ(KaioSpawn))
		spawn(10)
			BeenGivenStartingGrav=0
			Gravity()
			StartingGrav()
