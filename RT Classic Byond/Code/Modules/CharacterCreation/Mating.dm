...
obj/items/Genetic_Sequencer
	...
	verb/Sequence(mob/M in view(1))
		...
		if(!M.CanMate2())
			...
			return
		if(M.Sterile||M.Race=="Android"||M.Race=="Majin"||M.Dead||M.BeenSequenced>2)
			usr<<"You can not use this on them."
			return
...
mob/proc/MakeFakeBaby(obj/items/Genetic_Sequencer/A,obj/items/Genetic_Sequencer/B,mob/CC)
	...
	else if((B.GeneticRace=="Kaio"&&A.GeneticRace!="Kaio")||(B.GeneticRace!="Kaio"&&A.GeneticRace=="Kaio"))
	...
	if(C.Race!="Half-Saiyan") if(C.Race!="Part-Saiyan") if(C.Race!="Demigod")
		if(A.GeneticRace!="Undefined Class") C.Class="[A.GeneticRace]"
		else if(B.GeneticRace!="Undefined Class") C.Class="[B.GeneticRace]"
	C.BPMod=round(((A.GeneticBPMod+B.GeneticBPMod)/2),0.05)
	C.FBMMult=round(((A.GeneticFBMMult+B.GeneticFBMMult)/2),0.05)
	if(C.Race=="Half-Saiyan") C.FBMMult=1.7
...
//	C.PowMod=round(((B.GeneticPowMod+A.GeneticPowMod)/2),0.05)
	...
	if(C.Race==A.GeneticRace&&B.GeneticRace==A.GeneticRace)
		...
		LearnList1.Remove("Pantheon","Swell","Physics Simulation","Super Tuffle","Giant Mode","Makyo Transform","Majin","Mystic","Super Namekian","Post Human","Humanism")
	...
	if(B.GeneticRace=="Kaio"||A.GeneticRace=="Kaio")GeneticLearnList+="Mystic"
	...
	if(C.Race=="Half-Human")GeneticLearnList+="Post Human"
...
mob/proc/MakeBaby(mob/A,mob/B)
	...
	else if((B.Race=="Kaio"&&A.Race!="Kaio")||(B.Race!="Kaio"&&A.Race=="Kaio"))
		C.GeneticLearnList+="Mystic"
		C.Race="Demigod"
	...
	if(C.Race!="Half-Saiyan") if(C.Race!="Part-Saiyan") if(C.Race!="Demigod")
		if(A.Class!="Undefined Class") C.Class="[A.Class]"
		else if(B.Class!="Undefined Class") C.Class="[B.Class]"
	C.BPMod=round(((A.GeneticBPMod+B.GeneticBPMod)/2),0.05)
	C.FBMMult=round(((A.GeneticFBMMult+B.GeneticFBMMult)/2),0.05)
	if(C.Race=="Half-Saiyan") C.FBMMult=1.7
	C.Decline=(B.GeneticDecline+A.GeneticDecline)/2
...
//	C.PowMod=round(((B.GeneticPowMod+A.GeneticPowMod)/2),0.05)
	...
	if(C.Race==A.Race&&B.Race==A.Race)
		...
		while(LearnRemove>1&&LearnList1.len)
			...
			LearnRemove--
			C.GeneticLearnList+=Z
	if(B.Race=="Kaio"||A.Race=="Kaio")GeneticLearnList+="Mystic"
...
mob/proc
	CanMate()
		...
		if(MadeChildren>=4+(HasTheSeedIsStrong*2)+(HasBroodMother*4))
			...
			return 0
		if(EXPLifetime<500)
			src<<"You need atleast 550 lifetime exp."
			return 0
		/*if(Race=="Demon")
			src<<"Demons can not mate."
			return 0
		if(Race=="Kaio")
			src<<"Kaio can note mate."
			return 0*/
		if(SaveAge<2)
			src<<"Your save must be three years old before you can mate."
			return 0
		if(Race=="Post Human")
	...
	CanMate2()
		...
		if(EXPLifetime<550)
			...
			return 0
		if(SaveAge<2)
			src<<"Their save must be three years old before you can mate."
			return 0
		if(Sterile)
		...
		if(Age>Decline)  if(!HasCialis)
			...
			return 0
		if(Race=="Android")
			src<<"Androids can't do this."
			return 0
		if(RoidPower)
			src<<"They are steroids are preventing you from becoming fertile."
			return 0
...
obj/Mate
	...
	verb/Mate()
		...
		if(Year<3&&!usr.AdminCanMate)
			...
			return
		if(Year<LastUse+(1-(usr.HasTheSeedIsStrong*0.5))) if(!usr.HasBroodMother)
			usr<<"You cannot use this until year [LastUse+1-(usr.HasTheSeedIsStrong*0.5)]"
			return
		if(Mating) return
			...
			for(var/obj/Contact/C in usr.Contacts) if(C.Signature == A.Signature)
				if(C.familiarity>14.9)
					if(A.Asexual||A.gender=="Asexual")
						...
						return
					if(usr.Race=="Kaio"&&A.Race=="Demon")
						usr<<"Kaios can not mate with Demons."
						return
					if(usr.Race=="Demon"&&A.Race=="Kaio")
						usr<<"Kaios can not mate with Demons."
						return
					if(!A.CanMate()) return
