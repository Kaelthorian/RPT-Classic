		...
		for(var/obj/Resources/M in usr)
			if(M.Value > Actual)
				...
				if(CC=="Yes")
					...
					if(BPP)
						BPP.Cybernetic=1
						if(BPP.MaxHealth<=150) BPP.MaxHealth+=50 // if(BPP.MaxHealth=150) BPP.MaxHealth=150
						A.CyberLimb(BPP)
						A.Injure_Heal(200,BPP,1)
						A.Injure_Heal(200,BPP,1)
						M.Value-=Actual
						view(usr)<<"[usr] has successfully replaced [A]'s [C] with a cybernetic version."
						hearers(10,usr) << sound('rped.ogg',volume=20)
...
Skill/Technology/Replace_Limb
	...
	verb/Replace_Limb()
		...
		var/Cost = 105000000
		var/Actual = round(initial(Cost)/usr.Int_Mod*(1-(0.15*usr.HasDeepPockets)))
		usr << "Base cost for this skill is [Commas(Cost)] resources and a Heart Of The Mountain. Your intelligence means it costs [Commas(Actual)] resources for you."

		for(var/obj/Resources/M in usr)
			if(M.Value > Actual)
				if(locate(/obj/items/ingredients/Heart_Of_The_Mountain) in usr)
					for(var/obj/items/ingredients/Heart_Of_The_Mountain/AS in usr)
						var/list/Choices=new
						Choices+="Cancel"
						for(var/mob/player/PP in view(3)) Choices+=PP
				...
				usr << "You do not have [Commas(Actual)] resources or/and a Heart Of The Mountain resources to spare in order to use Replace Limb."
				return
