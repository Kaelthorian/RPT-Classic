...
mob/proc/UpdateStats()
	set background=1
	set waitfor=0
//	if(!src.GodKiActive)winset(src.client,"GODKI","is-visible=false")
	if(src.LethalCombatTracker) winset(src.client,"lethalcombat","is-visible=true")
	else winset(src.client,"lethalcombat","is-visible=false")
