mob/proc/EventKillReward(mob/M,mob/C)
	/*M.AllOut("You were rewarded EXP and temporary BP for killing [C]!")
...
//	if(!locate(/obj/RankChatECPool) in src) src.contents+=new /obj/RankChatECPool
	...
	if(!locate(/obj/Set_Willpower) in src) src.contents+=new /obj/Set_Willpower
...
//			src<<"Added [X] to skill set"
	...
	var/Skill/LB = new /Skill/Buff/LimitBreaker
	LB.RankKit=1
	contents+=LB
...
*/
	...
	var/zz= 2
	switch(input(src.client,"Are you sure?") in list ("Yes", "No",))
		if("Yes")
			unSummonX = x
			unSummonY = y
			unSummonZ = z
	...
	alert(src,"You are an event character. You are expected to contribute to conflict based RP and are expected to be killed off or defeated within 3-5 days of making(If you are a minor EC).Do not abuse , we trust you, and remember to have fun and make everyone else have fun too.")
...
obj/Set_Willpower
	desc="This will let you alter your Will Power."
	verb/SetEC_Will_Power()
		set category="Other"
		var/NewWillPower=input(usr,"What maximum Will Power would you like to set yourself to? (100 - 170)") as num
		if(NewWillPower<100) NewWillPower=100
		if(NewWillPower>170) NewWillPower=170
		usr.MaxHealth=NewWillPower
		usr.MaxWillpower=NewWillPower
		usr.Willpower=NewWillPower
		usr.Health=NewWillPower
		usr.BuffOut("Your maximum Will Power has been set to [usr.MaxWillpower].")
		alertAdmins("[key_name(usr)] altered their Willpower to [usr.MaxWillpower].")
