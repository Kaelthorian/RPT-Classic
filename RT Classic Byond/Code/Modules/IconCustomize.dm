...
mob/verb/AdjustXY()
	...
	if(C=="Self")
		...
		if(istype(O,/obj/items/Magic_Ball)) return
		...
		if(istype(O,/obj/Crystal/)) return
...
//		if(istype(O,/obj/items/Elixir_Of_Respec))return
		...
		if(istype(O,/obj/items/Artifact)) return
		if(istype(O,/obj/Crystal/)) return
		if(usr)
...
//		set src=usr.contents
	...
	verb/Change_Object_XY_Or_Layer(obj/O as obj in view(usr))
		...
		if(O.pixel_y>128)O.pixel_y=128
		if(O.pixel_y<-128)O.pixel_y=-128
		var/layP=input("Choose a layer for [O] 2-10 0 for default") as num
		if(layP) O.layer=layP
		else O.layer=initial(O.layer)
		if(O.layer>10) O.layer=10
		if(O.layer<1) O.layer=2
	verb/Change_Object_Icon(obj/A as obj in view(usr))
		...
		if(length(ICO) > iconsize)
			...
			if(istype(A,/obj/items/Magic_Ball)) return
			...
			if(istype(A,/obj/items/Artifact)) return
			if(istype(A,/obj/Crystal/)) return
