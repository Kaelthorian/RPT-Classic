mob/proc/Status()
	...
	while(src&&(client||adminObserve||TestChar))
		...
		if(EXPLifetime >= ((Year*50)+150) && !RaceEraScore)
			EraScore(src)
		...
		spawn if(src) if(!src.RPMode)
			...
			if(GuardBreaking)
				GuardBreaking--
				if(!GuardBreaking)for(var/mob/player/M in view(src)) M.BuffOut("[src] is no longer guard breaking.")
			if(HeartAiming)
				HeartAiming--
			...
			if(ChakraBlocked)
				...
				//	winset(src.client,"ChakraB","is-visible=false")
			if(Wing_Clipped)
				Wing_Clipped--
				if(!Wing_Clipped)
					for(var/mob/player/M in view(src)) M.BuffOut("[src] is no longer wing clipped.")
				//	winset(src.client,"Wing_Clipd","is-visible=false")
			spawn()
				for(var/BodyPart/P in src)
		...
		if(BaseWeight>1+((StrMod+EndMod)*2)) BaseWeight=1+((StrMod+EndMod)*2) //src.KO("wearing too heavy of weights")
		Weight=(BaseWeight/(StrMod+EndMod))
		if(Weight<1) Weight=1

		//Gravity
		var/turf/RR=src.loc
		src.Gravity=RR.Gravity

//		if(src.Gravity <= 0) src.Gravity = 1
//		if(src.Gravity>src.GravMastered) src.Gravity_Gain()
//		src.GravMulti = 1+((max(Gravity,GravMastered)/25)/25)
...
//				SpaceCheck()
			...
			EmotionCheck()
			src.Bio_Forms()																		//I know this run every 1 second but I can't find a better place.This will check if the bio android have or not the reqs for transformation , if he does then he will transform.
			if((winget(src.client,"emoteW","is-visible")=="true")) winset(src.client,"WordCounter","text=\"Word Count: [wordcount(winget(src.client, "emoteW.emoteinput", "text"))]\"")
		sleep(15)
