	...
	Weights
		...
		Click() if(locate(src) in usr)
			...
			if(suffix)
				...
				usr.Weights=0
				usr<<"You take off the [src]. Your max lift is [Commas(round((usr.StrMod+usr.EndMod)*2))] pounds"
			...
			if(!suffix&&Weights>=3)
				...
				return
			if(!suffix&&(CurrentWeight+Weight)>((usr.StrMod+usr.EndMod)*2))
				usr<<"Putting this on would exceed your maximum lift of [Commas(round((usr.StrMod+usr.EndMod)*2))] pounds. You cannot use it."
				return
			if(!suffix)
				suffix="*Equipped*"
				usr.overlays+=icon
				...
				usr.Weights=1
				usr<<"You put on the [src]. Your max lift is [Commas(round((usr.StrMod+usr.EndMod)*2))] pounds"
