...
//		if(Belongs_To.attacking==2) Belongs_To.DrainKi(src,1,(((kiDrain*DrainMult)*0.5)/maxSteps),0) // Drain their energy
	...
	Bump(atom/Z)
		...
		if(isobj(Z))
			if(istype(Z,/obj/ranged))
				...
				if(isnum(Z.Health))if(isnum(Power))if(isnum(Damage))
					if(istype(Z,/obj/TrainingEq/Punchometer))
						...
						Z.TakeDamage(src,Damage, "Beam","energy")
						if(Z.Health<=0)
							SmallCrater(Z)
							DeleteSaveItem(Z)
							del(Z)
			...
			if(isnum(A.Health))
				...
				var/EndRed=0
				if(Belongs_To) EndRed=((Belongs_To.HasThisDrill*0.2)*A.End)
				var/DMG = Damage/((A.BP+EMPBP)*(A.End-EndRed))
				A.TakeDamage(Belongs_To, DMG, "[src]","energy")
				if(CausesBurns&&prob(20)) A.BurnDamage(Belongs_To,"Smolder Burn",DMG*0.25)
			...
			if(!Piercer)
				if(src)
					del(src)
					return FALSE
			A.LastHitter = usr.name															//Read the hit him with an autoattack and save it in LastHitter
			A.HitterListCheck(usr.name)														//Runs the HitterListCheck proc from Better Anger while aiming to the one who hitted..
	...
	proc/beamVariables(Skill/Attacks/A,mob/P,Spread,Special=0)
		...
		if(P.MaskOn) for(var/obj/items/Magic_Mask/MM in P) if(MM.suffix&&MM.Durability>0)
			...
			MM.DurabilityCheck(P)
			break
		if(MaskDamage>usr.BP*0.2)MaskDamage=usr.BP*0.2
...
mob/proc
	...
	Beam_Macro(Skill/Attacks/Beams/Beam/O,var/Spread=0,var/Special=0)
		...
		if(!O.charging&&!attacking&&O.BeamMode==1)
			...
			hearers(3,usr) << sound(pick('Charging.wav','Charging2.wav'),volume=10)
			...
			hearers(3,usr) << sound('BeamFire2.wav',volume=20)
	...
	BeamCharge(Skill/Attacks/A,Special)
		...
		spawn while(A.charging&&!A.streaming)
			...
			usr.DrainKi(A,1,(A.KiReq*(A.chargelvl*A.ChargeRate)/min(0.5,1))*(1+(LoopsCompleted/3)))
			LoopsCompleted++
			if(!Special) sleep((Refire*A.ChargeRate)/(max(0.8,1)))
			else sleep((10*A.ChargeRate)/(max(0.8,1)))
...
//			if(A.Experience>100) A.Experience=100
			...
			sleep(round(A.MoveDelay/(max(0.8,1))))
			...
			DrainKi(A,1,5+(((B.kiDrain*(Spread+1))*A.chargelvl))*(1+(LoopsCompleted/4)))// ensures that even 'point blank' firing drains energy
			...
			if(Spread==2)
				...
				if(C) spawn if(C)
					C.shoot()
					walk(C,dir,A.MoveDelay/(max(0.8,1)))
				...
				if(D) spawn if(D)
					D.shoot()
					walk(D,dir,A.MoveDelay/(max(0.8,1)))
			walk(B,dir,A.MoveDelay/(max(0.8,1)))
			LoopsCompleted++
	...
	BeamStop(Skill/Attacks/A)
		...
		if(KOd==0) move=1
		if(!istype(A,/Skill/Attacks/Beams/Beam)) A.CD=src.Refire*1.5
		else A.CD=src.Refire*0.5
		A.CDTick(src, src.HasCooldownMastery)
