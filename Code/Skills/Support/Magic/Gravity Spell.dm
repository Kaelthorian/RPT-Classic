	...
	Gravity_Well
		...
		verb/Gravity_Well()
			...
			if(usr.Magic_Level >= 10)
				...
				var/Grav=input("Gravity Multiplier: Costs [Actual] mana per 1x  (Your Limit is [(usr.Magic_Potential*GravTune)]) and the max the game will ever reach is 240") as num
//				if(Grav>GravLimit) Grav=GravLimit
//				if(Grav>(usr.Magic_Level-50)*6) Grav=(usr.Magic_Level-50)*6
				if(Grav>(usr.Magic_Potential*GravTune)) Grav=(usr.Magic_Potential*GravTune)
				if(Grav>240) Grav=240
				if(Grav<1) Grav=1
				if(!Grav) Grav=1
				if(Grav<=1) return
				Grav=round(Grav)
