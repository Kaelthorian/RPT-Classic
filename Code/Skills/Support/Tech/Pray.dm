Skill/Technology/Pray
	desc="Pray to the gods , you may be heard."
	verb/Pray()
		set category="Other"
		var/Cost = 390000000																													//Skill price
		var/tmp/list/KaioOptions = new/list()																									//We make a list for kaios
		var/KaioAFK																																//check for kaios afk
		var/Actual = round(initial(Cost)/usr.Int_Mod*(1-(0.15*usr.HasDeepPockets)))																//Actual real price with the discounts
		KaioOptions.Add("Cancel")
		usr << "Base cost for this skill is [Commas(Cost)] resources. Your intelligence means it costs [Commas(Actual)] resources for you."		//We tell the user how much it will be
		for(var/mob/player/A)																													//For all the players
//			world<<"For all the Players"																										//Debug tool
			if(A.Race=="Kaio" && A.z==11 && A.SummonedBy==null && !A.Dead)																		//Who are a kaios , are in the AL and were not summoned yet
...
//					world<<"KaioAFK:[KaioAFK]"																									//Debug tool
		if(KaioAFK>0 && KaioOptions.len == 0)																									//If all the kaios are afk.
			usr<<"You pray as hard as you can but they are in a deep sleep."																	//Let the player know about it
			return																																//And we stop
		if(KaioOptions.len == 0)																												//If there are no kaios at all , not even afk
			usr<<"You pray as hard as you can but no one can hear your words."																	//Let the player know about it
			return																																//And we stop
		...
		for(var/mob/player/A)																													//For all the players in the game
			if(A.name==KaioPick)																												//If your name = the one the summoner picked
				...
				if(A.isGrabbing)
					...
					return
				for(var/obj/Resources/M in usr)																									//Time to pay
					if(M.Value > Actual)																										//You have enough money for it ?
						if(usr.Confirm("Attempt to summon a Kaio for [Actual] resources?"))														//Good , you do , want to pay ?
//							world<<"Payday"																										//Debug Tool
							M.Value -= Actual																									//Paying...
							view(src)<<"[usr] begins to pray!"																					//PRAY PRAY PRAY PRAY PRAY
							usr.saveToLog("| | ([usr.x], [usr.y], [usr.z]) | [key_name(usr)] summoned a Kaio")									//Logs
