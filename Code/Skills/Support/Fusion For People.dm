...
mob/proc/MakeFusion(mob/A,mob/B)
	...
	C.StrMod=(0.85*max(B.StrMod,A.StrMod))+(0.35*(min(B.StrMod,A.StrMod)))
	C.EndMod=(0.85*max(B.EndMod,A.EndMod))+(0.35*(min(B.EndMod,A.EndMod)))
	C.SpdMod=(0.85*max(B.SpdMod,A.SpdMod))+(0.35*(min(B.SpdMod,A.SpdMod)))
	C.OffMod=(0.85*max(B.OffMod,A.OffMod))+(0.35*(min(B.OffMod,A.OffMod)))
	C.DefMod=(0.85*max(B.DefMod,A.DefMod))+(0.35*(min(B.DefMod,A.DefMod)))
...
//	C.UpdateStats("All")
	...
	C.OldMob=A
	spawn() C.RPMode()
	alert(C,"Spend your milestones!","Points refunded")
