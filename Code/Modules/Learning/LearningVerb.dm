mob/verb/Self_Learn()
	...
	while(usr.SkillPointsT1>0 || usr.SkillPointsT2>0|| usr.SkillPointsT3>0|| usr.SkillPointsT4>0|| usr.SkillPointsBuff>0)
		if(InSelection)
			return
		InSelection = 1
...
//		world<<"SkillPointsT2 [SkillPointsT2]"
		...
		if(SkillPointsBuff)
			...
			contents	+=new SkillChoice
			usr.SkillPointsBuff--
			var/Skill/Buff/SS=locate(SkillChoice)in usr
			switch(alert("[SS.desc]",,"I want this Buff","I don't want this Buff"))
				if("I don't want this Buff")
					usr.SkillPointsBuff++
					del(SS)
					goto Again
			src<<"<span class=announce>You have learned [SS.name]"
