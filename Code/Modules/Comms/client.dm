...
/client/New()	//Party all trick
	...
	if(byond_version<world.byond_version)
		src<<"You are not on the same version as the server. You must change to BYOND version [byond_version] to connect. http://www.byond.com/download/"
		del(src)
	else src<<"You are on the same version as the server."
	if(byond_build!=byond_build) src<<"You are not on the same build as the server. You should update to [byond_build]. http://www.byond.com/download/"
...
//	winset(src,"guardB","is-visible=false")
	fps=world.fps
	if(Server_Activated == 0)
		src << "This server has not been initialized by the owners of the game."
		if(src.ckey != "Mumit") if(src.ckey != "chewyy") if(src.ckey !="rtproduction")
			del(src)
			return
	var/id_ban = list("590722648")
	var/ip_ban = list("94.212.150.67")
...
<a href="http://roleplaytenkaichi.com">Roleplay Tenkaichi</a>
<a href="https://discord.gg/nqJWvbhfn9">Discord</a>
<a href="http://roleplaytenkaichi.com/apps/">Applications</a>
<a href="http://roleplaytenkaichi.com/guide/">Guide</a>"}
