...
Skill/Support/Counterpart
	...
	with you and one other person. The effect is permanent and irreversible.The counterpart with the lowest training experience will match the other counterpart's \
	...
	verb/Set_Counterpart(mob/A in oview(1))
		...
		if(A.Race!=usr.Race)
			...
			return
		if(A.Counterpart || usr.Counterpart)
		...
		switch(input(A,"[usr] wants to be your counterpart. Do you want this?") in list("No","Yes"))
			if("Yes")
				...
				A.HiveMind= rand(1,10000)
				usr.HiveMind=A.HiveMind
				A.contents+=new/Skill/Misc/HiveMind
...
obj/Contact
	...
	//verb/View_Notes() usr.AllOut(Notes)
/*
	verb/Change_Relation()
	verb/Update_Contact()
		for(var/mob/player/M in oview(6,src)) if(M.client)
...
/*		spawn(2)
			if(C)
				...
				if(usr.Rank) Choices+="Disciple(+)"
				Choices.Add("Family","Friend","Sibling","Rival","Neutral","Enemy","Hated","Mentor","Spouse","Child","Delete Contact")
				var/RC=input("Choose a Relationship with [M]") in Choices
				if(RC=="Delete Contact") if(usr.Confirm("Delete [M] contact?")) del(C)
				else C.relation=RC */
...
obj/items/FriendshipBracelet
	...
	verb/Destroy()
		set category=null
		if(!src.suffix)
			if(usr.Confirm("Delete [src]? (You will not be refunded)"))
				if(!src.suffix)
					if(src in usr)
						DeleteSaveItem(src)
						del src
