...
proc/FactionPowerCheck()
	for(var/mob/player/A in Players)							//For all the players in the list of players
		if(Z1ControllingRuler == A.real_name)					//If the Z1controllingRuler variable is the same as the name of one of them
			A.RPPower=1											//Set their RP power to 1 , to remove previous RPP from other changes.
			A.RPPower*=(1+(FactionPower-1)*(Z1Tax/90))			//Set the RPP to whatever our Faction power is at.
		if(Z2ControllingRuler == A.real_name)
			A.RPPower=1
			A.RPPower*=(1+(FactionPower-1)*(Z2Tax/90))
		if(Z3ControllingRuler == A.real_name)
			A.RPPower=1
			A.RPPower*=(1+(FactionPower-1)*(Z3Tax/90))
		if(Z4ControllingRuler == A.real_name)
			A.RPPower=1
			A.RPPower*=(1+(FactionPower-1)*(Z4Tax/90))
		if(Z5ControllingRuler == A.real_name)
			A.RPPower=1
			A.RPPower*=(1+(FactionPower-1)*(Z5Tax/90))
		if(Z6ControllingRuler == A.real_name)
			A.RPPower=1
			A.RPPower*=(1+(FactionPower-1)*(Z6Tax/90))
		if(Z7ControllingRuler == A.real_name)
			A.RPPower=1
			A.RPPower*=(1+(FactionPower-1)*(Z7Tax/90))

mob/Admin1/verb
	Faction_Power()
		set category="Admin Other"
		FactionPower=input("Faction Leaders -MAX- power (At 90% tax and in RPP).Current is [FactionPower]") as num
		logAndAlertAdmins("[key_name(usr)] changed the Faction Power to [FactionPower] RPP.")
		FactionPowerCheck()
mob/Admin1/verb
	Faction_Power_Check()
		src << "[FactionPower]"
mob/var/TaxCD
var/global/FactionPower=0
///////////////////////////////////////////////////////////////////
////////////////////////////HTML WINDOW////////////////////////////
///////////////////////////////////////////////////////////////////
obj/Faction/proc/FactionRefresh()
	Faction()
...
<div class="sidebar">
	<A HREF='?src=\ref[src];Recruit=\ref[src]'>Recruit</A>
	<A HREF='?src=\ref[src];Notes=\ref[src]'>Edit Notes</A>
	<A HREF='?src=\ref[src];ResourcesPay=\ref[src]'>Resources Pay</A>
	<A HREF='?src=\ref[src];ManaPay=\ref[src]'>Mana Pay</A>
	<A HREF='?src=\ref[src];Name=\ref[src]'>Rename Faction</A>
	<A HREF='?src=\ref[src];MemberRank=\ref[src]'>Change Member Rank</A>
	<A HREF='?src=\ref[src];FactionIcon=\ref[src]'>Change Faction Icon</A>
	<A HREF='?src=\ref[src];SwitchLeaders=\ref[src]'>Change Leadership</A>
	<A HREF='?src=\ref[src];BootMember=\ref[src]'>Boot Member</A>
	<A HREF='?src=\ref[src];LeaveFaction=\ref[src]'>LeaveFaction</A>
	<A HREF='?src=\ref[src];Checkpoint=\ref[src]'>Checkpoint</A>
</div>
<div class="main">
<center><h2><A HREF='?src=\ref[src];Notes=\ref[src.notes]'></i>[name]</a></h2></center>
<tr><td >[notes]</td></tr><br>
<tr><td>Pay Check in Resources: </td><td>[PayGrades.Join(", ")]</td></tr><br>
<tr><td>Pay Check in Mana: </td><td>[MPayGrades.Join(",")]</td></tr><br>
<tr><td>Members Online: </td><td>[MembersUpdate().Join(",")]</td></tr><br>
</div>
</body>
</html>
	...
	"}
		usr<<browse(Z,"window='Faction';size=650x800")

/obj/Faction/Topic(href, href_list)
	..()
	if(href_list["ResourcesPay"])
		...
		return
	if(href_list["ManaPay"])
		if(leader|rank>=4)
			MPayGrades[1]=input("Choose the resource pay for rank 1") as num
			MPayGrades[2]=input("Choose the resource pay for rank 2") as num
			MPayGrades[3]=input("Choose the resource pay for rank 3") as num
			MPayGrades[4]=input("Choose the resource pay for rank 4") as num
			version++
			FactionRefresh()
			return
		usr<<"Your rank is not high enough for this action."
	...
	if(href_list["SwitchLeaders"])
		...
		usr<<"Your rank is not high enough for this action."
		return
	if(href_list["LeaveFaction"])
		del(src)

	if(href_list["Checkpoint"])
		if(leader)
			var/list/CPs=list()
			if(Z1ControllingRuler==usr.name||usr.FactionApproved(Z1FactionCode,1)) CPs+="Earth"
			if(Z2ControllingRuler==usr.name||usr.FactionApproved(Z2FactionCode,1)) CPs+="Namek"
