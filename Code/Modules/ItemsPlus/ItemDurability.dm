obj/items
	...
	proc/DurabilityCheck(mob/user)
		...
		if(Durability<=1||Health<=1)
			...
			if(istype(src,/obj/items/Artifact))user.ArtifactRemove()
	...
	proc/EquipmentDescAssign()
		if(istype(src,/obj/items/Armor))
			...
			if(A.KineticBarrier) A.desc="<br>[Commas(A.Health)] Health Armor. Wearing it will trade [A.Spd*100]% Speed for [A.KineticBarrier+100]% BP towards endurance calculations. (Armor Mastery adds +7% End)."
			if(AvoidsCrits)A.desc+="<br> This armor will block critical strikes[AvoidsCrits>1?" and prevent auracite energy drain.":"."]"
