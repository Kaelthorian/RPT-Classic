...
src.path_list = AStar(src.loc, target.loc, /turf/proc/GetAdjacent(), /turf/proc/Distance)
...
/turf/proc/CardinalTurfsWithAccess(var/obj/items/Door_Pass/ID)
	var/L[] = new()
...
/proc/LinkBlockedWithAccess(turf/A, turf/B, var/obj/items/Door_Pass/ID)
	...
	for(var/obj/O in B)
		if(O.density && !istype(O, /obj/Door))
			return 1
...
/proc/DirBlockedWithAccess(turf/loc, var/dir, var/obj/items/Door_Pass/ID)
//	for(var/obj/window/D in loc)	//Window doors!
//		if(!D.density)			continue
//		if(D.dir == SOUTHWEST)	return 1
//		if(D.dir == dir)		return 1

	for(var/obj/Door/D in loc)
		if(!D.density)			continue
