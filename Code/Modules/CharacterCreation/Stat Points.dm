...
mob/proc/Stat_Point_Window_Refresh()
	...
	if(src) if(client)
		...
		winset(src.client,"bodyc","text=\"[Size]\"")




mob/proc/Raise_Energy(Amount=1)
	if(Race == "Alien")KiMod+=(Minimum_Stats["Eff"]*0.1)*Amount*(1-(Total_Stats_Energy*0.03))
	else KiMod+=(Minimum_Stats["Eff"]*0.1)*Amount*(1-(Total_Stats_Energy*0.1))
mob/proc/Raise_Speed(Amount=1)
	if(Race == "Alien")SpdMod+=(Minimum_Stats["Spd"]*0.1)*Amount*(1-(Total_Stats_Speed*0.03))
	else SpdMod+=(Minimum_Stats["Spd"]*0.1)*Amount*(1-(Total_Stats_Speed*0.1))
mob/proc/Raise_Strength(Amount=1)
	if(Race == "Alien")StrMod+=(Minimum_Stats["Str"]*0.1)*Amount*(1-(Total_Stats_Strength*0.03))
	else StrMod+=(Minimum_Stats["Str"]*0.1)*Amount*(1-(Total_Stats_Strength*0.1))
mob/proc/Raise_Durability(Amount=1)
	if(Race == "Alien")EndMod+=(Minimum_Stats["End"]*0.1)*Amount*(1-(Total_Stats_Endurance*0.03))
	else EndMod+=(Minimum_Stats["End"]*0.1)*Amount*(1-(Total_Stats_Endurance*0.1))
mob/proc/Raise_Offense(Amount=1)
	if(Race == "Alien")OffMod+=(Minimum_Stats["Off"]*0.1)*Amount*(1-(Total_Stats_Off*0.03))
	else OffMod+=(Minimum_Stats["Off"]*0.1)*Amount*(1-(Total_Stats_Off*0.1))
mob/proc/Raise_Defense(Amount=1)
	if(Race == "Alien")DefMod+=(Minimum_Stats["Def"]*0.1)*Amount*(1-(Total_Stats_Def*0.03))
	else DefMod+=(Minimum_Stats["Def"]*0.1)*Amount*(1-(Total_Stats_Def*0.1))
mob/proc/Raise_Recovery(Amount=1)
	if(Race == "Alien")BaseRecovery+=(Minimum_Stats["Rec"]*0.1)*Amount*(1-(Total_Stats_Recov*0.03))
	else BaseRecovery+=(Minimum_Stats["Rec"]*0.1)*Amount*(1-(Total_Stats_Recov*0.1))
...
mob/proc/CheckRedoneStats()
	if(StrMod>SoftStatCap*1.2)StrMod=SoftStatCap*1.2
	if(EndMod>SoftStatCap*1.2)EndMod=SoftStatCap*1.2
	if(SpdMod>SoftStatCap*1.2)SpdMod=SoftStatCap*1.2
	if(OffMod>SoftStatCap*1.2)OffMod=SoftStatCap*1.2
	if(DefMod>SoftStatCap*1.2)DefMod=SoftStatCap*1.2
...
mob/proc/RevertAll()
	...
	if(Race=="Kanassa") Precognition=1
	SNj=0
	HasThornsOn=0
	Cancel_Olympus()
...
mob/proc/OverlayCleanse()
	overlays.Remove(/Icon_Obj/Cloak/UIAura,/Icon_Obj/Cloak/LSSJCloak,/Icon_Obj/Cloak/BlackCloak,/Icon_Obj/Cloak/WhiteCloak,/Icon_Obj/Cloak/PurpleCloak,/Icon_Obj/Cloak/BlueCloak,/Icon_Obj/Cloak/PinkCloak,/Icon_Obj/Cloak/DarkAura)
