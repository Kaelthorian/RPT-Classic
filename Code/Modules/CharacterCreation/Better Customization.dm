...
mob/verb/Show_Milestones()
	...
	for(var/Milestone/MP in src)
		...
		src << output("[MP.desc] <br>(Cost [MP.PointsCost] MP[MP.Refundable==0?" Non-Refundable!":""])<br>(Year Acquired: [MP.YearAcquired])","Milestones.Grid:2,[Row]")
...
mob/proc
	...
	Grid(var/Z,var/X,var/E)
		...
		if(Z=="CreationKanassa")
			Kanassa_List=list()
			winset(src.client,"Grid.SelectedCustomize","text=\"Kanassa Icon\"")
			for(var/B in typesof(/Icon_Obj/Customization/Kanassa_Icons)) if(B!=/Icon_Obj/Customization/Kanassa_Icons) Kanassa_List+=new B
			for(var/A in Kanassa_List)
				Row++
				src<<output(A,"GridX:1,[Row]")
