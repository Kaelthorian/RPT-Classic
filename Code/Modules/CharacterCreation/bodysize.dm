...
mob
	verb
		RedoBodySize()
			...
			if(Race=="Undefined")
				...
				return
			if(Race=="Android")
				usr<<"Androids no longer have body sizes."
				return
			if(Offspring)
				usr<<"Offspring may not change body size."
				return
	...
	proc
		...
		BodyStatGive()
			...
			KiMod=round(KiMod,0.1)
			StrMod=round(StrMod,0.1)
			EndMod=round(EndMod,0.1)
			SpdMod=round(SpdMod,0.1)
			OffMod=round(OffMod,0.1)
			DefMod=round(DefMod,0.1)
			BaseRegeneration=round(BaseRegeneration,0.1)
			BaseRecovery=round(BaseRecovery,0.1)
		...
		GiveBodyParts()
			...
			if(!locate(/BodyPart/Throat) in src) src.contents+=new/BodyPart/Throat(src)
//			if(!locate(/BodyPart/Throat) in src) if(Race!="Android")  if(Race!="Majin") src.contents+=new/BodyPart/Throat(src)
			...
			if(!locate(/BodyPart/Ears) in src) src.contents+=new/BodyPart/Ears(src)
//			if(!locate(/BodyPart/Ears) in src) if(Race!="Android") if(Race!="Majin") src.contents+=new/BodyPart/Ears(src)
			...
			if(!locate(/BodyPart/Tail) in src) if(Race=="Saiyan"||Race=="Half-Saiyan") contents+=new/BodyPart/Tail(src)
