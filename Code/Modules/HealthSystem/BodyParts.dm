...
//		if(usr.KeepsBody==0) if(usr.Dead) return
	...
	Head
		...
		InjuryHealed(mob/M,forced=0)
			...
			else if(Status=="Missing"&&forced) Status="Maimed"
			else
				if(M.Critical_Head)
					M.Critical_Head = 0
					//M.Int_Mod*=2
...
//			Moon_Used=0
		...
		verb/Custom_Oozaru_Icon()
			//set category="Other"
		/*	if(usr.OozaruIcon.icon!='OozaruNew.dmi')
				usr.OozaruIcon.icon='OozaruNew.dmi'
				return*/
...
//				return
			...
			if(usr.EXPLifetime<300)
				usr<<"You require atleast 300 lifetime EXP in order to be able to injure or rob someone or to kill someone."
				return
			...
			if(A.Willpower<=30)
				Choices.Add("Loot")
				if(usr.EXPLifetime>=300&&A.Willpower<=0) Choices.Add("Kill")
			...
			if(usr.Confirm("[Result]?")) if(A.KOd) if(A in range(1,usr))
				if(Result=="Kill" && !A.immortal)
					A.Death(usr,Killer=A)

				if(Result=="Seize Control Point")
					if(usr.Majinized==1)
						usr<<"Your mind is too broken to take lead the planet."
						return
					usr.TakeCP(A)
				...
				if(Result=="Humiliate") A.Humiliate(usr)
					...
					for(var/obj/items/Artifact/R in A) if(R.Stealable) Lootables+=R
					Lootables+="Cancel"
					var/obj/C=input("Loot what from [A]?") in Lootables
					if(C=="Cancel") return
					else
						view(usr)<<"[usr] steals [C] from [A]"
						if(istype(C,/obj/Resources))
							for(var/obj/Resources/R in usr)
								for(var/obj/Resources/CC in A)
									R.Value+=CC.Value
									CC.Value=0
						else if(istype(C,/obj/Mana))
							for(var/obj/Mana/R in usr)
								for(var/obj/Mana/CC in A)
									R.Value+=CC.Value
									CC.Value=0
							...
							if(istype(C,/obj/items/Artifact))
								A.ArtifactSkillRemove()												//Artifacts here
								usr.ArtifactSkillAdd()
					...
					for(var/obj/Resources/R in A) Lootables+=R
					for(var/obj/Mana/R in A) Lootables+=R
					if(A.Willpower<=50) // Mumit tweak, lets herans steal all items below 50
						for(var/obj/items/R in A) if(R.Stealable) Lootables+=R
					Lootables+="Cancel"
					var/obj/C=input("Loot what from [A]?") in Lootables
					if(C=="Cancel") return
...
//		if(KeepsBody==0&&Dead) return
	...
	BPDamage(mob/M,Damage,rate=1)
		...
		else src.Injure_Hurt(Damage,"Random","???")
