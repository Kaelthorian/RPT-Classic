...
mob/proc/WillpowerFailure()
	if(Willpower<=0&&!WillpowerFailure)
		...
		src<<"You may choose to die at any point in time outside of a lethal situation while at 0 Willpower.  In order to die, please Ahelp with a request and explanation of the circumstances."
...
mob/var/tmp/WPRC=0
mob/proc/WillpowerRestoreCheck() if(Willpower<=0&&!LethalCombatTracker)
	Willpower=1
	Un_KO()
	WillpowerFailure=0
