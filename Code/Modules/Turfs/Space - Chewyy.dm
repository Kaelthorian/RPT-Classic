		...
		if(!Ship)
			...
			for(var/obj/Controls/MKIControls/B in world)
				if(B.Ship) FoundShip++
					...
					B.Ship=FoundShip
					break
			if(!Ship) view(src)<<"An available interior could not be found. Ahelp."*/
	MKIControls
		...
		verb/Enter_Ship()
			...
			if(Ship=="Ardent"||Ship=="Icebreaker")
				for(var/obj/Airlock/C in world) if(C.Ship==src.Ship)
					...
					usr.InShip=Ship
			else for(var/obj/Airlock/C in world) if(C.Ship==src.Ship)
				view(usr)<<"[usr] enters the ship"
				usr.loc=locate(C.x,C.y-1,C.z)
				usr.InShip=Ship
		Ardent
...
obj/Ships/verb/Knocking()														//Knocking verb
	set src in view(1)															//Set it so you can use it only if you are 1 tile away
//	world<<"WORKING CLICK"														//debug tool
	for(var/obj/Controls/PodControls/Q in world)								//For all the pod controls in the world
//		world<<"WORKING SEARCH"													//debug tool
		if(Q.Ship==src.Ship)													//If the variable Ship is the same as the source (Aka the ship/pod)
//			world<<"WORKING DINGDONG"											//debug tool
			range(10,Q) << "<font color=#FFFF00>There is a knock at the door!"	//Ding dong
			range(10,src) << "<font color=#FFFF00>There is a knock at the door!"//Ding FUCKING DONG I SAID
	for(var/obj/Controls/MKIControls/Q1 in world)								//For all the pod controls in the world
//		world<<"WORKING SEARCH"													//debug tool
		if(Q1.Ship==src.Ship)													//If the variable Ship is the same as the source (Aka the ship/pod)
//			world<<"WORKING DINGDONG"											//debug tool
			range(10,Q1) << "<font color=#FFFF00>There is a knock at the door!"	//Ding dong
			range(10,src) << "<font color=#FFFF00>There is a knock at the door!"//Ding FUCKING DONG I SAID
	for(var/obj/Controls/MKIIControls/Q2 in world)								//For all the pod controls in the world
//		world<<"WORKING SEARCH"													//debug tool
		if(Q2.Ship==src.Ship)													//If the variable Ship is the same as the source (Aka the ship/pod)
//			world<<"WORKING DINGDONG"											//debug tool
			range(10,Q2) << "<font color=#FFFF00>There is a knock at the door!"	//Ding dong
			range(10,src) << "<font color=#FFFF00>There is a knock at the door!"//Ding FUCKING DONG I SAID
	for(var/obj/Controls/MKIIIControls/Q3 in world)								//For all the pod controls in the world
//		world<<"WORKING SEARCH"													//debug tool
		if(Q3.Ship==src.Ship)													//If the variable Ship is the same as the source (Aka the ship/pod)
//			world<<"WORKING DINGDONG"											//debug tool
			range(10,Q3) << "<font color=#FFFF00>There is a knock at the door!"	//Ding dong
			range(10,src) << "<font color=#FFFF00>There is a knock at the door!"//Ding FUCKING DONG I SAID
