...
mob/proc/
	Assign_Control_Point(mob/M as mob in Players)
		...
		if(M.Majinized == 1)
			M<<"Your mind is too broken to lead someone."
			return
...
mob/proc/TakeCP(mob/A)
	if(locate(/Skill/Misc/FactionLeaderCommands) in A)
		...
		if(A.Majinized == 1)
			A.Majinized=0
			A<<"As you take the control point you feel how your mind finaly reach inner peace."
			usr.saveToLog("| | ([A.x], [A.y], [A.z]) | [key_name(A)] | [key_name(A)] is no longer Majinized due taking the CP.\n")
		logAndAlertAdmins("[key_name(src)] took control of [CP] from [A]  for [CF].")
		logAndAlertAdmins("[key_name(A)] is no longer Majinized due taking the CP.\n")
	else
		var/list/CPs=list()
		if(Z1ControllingRuler==A.name||A.FactionApproved(Z1FactionCode,4)) CPs+="Earth"
		if(Z2ControllingRuler==A.name||A.FactionApproved(Z2FactionCode,4)) CPs+="Namek"
...
<div class="main">
<center><h2>Universe Taxes</h2></center>

<br>Earth Tax =[Z1Tax]
<br>Earth ControllingFaction=[Z1ControllingFaction]
<br>Earth ControllingRuler=[Z1ControllingRuler]
<br>Earth FactionCode=[Z1FactionCode]
<br>Earth ReserveResources=[Z1ReserveResources]
<br>Earth ReserveMana=[Z1ReserveMana]
<br>
<br>
<br>Namek Tax=[Z2Tax]
<br>Namek ControllingFaction=[Z2ControllingFaction]
<br>Namek ControllingRuler=[Z2ControllingRuler]
<br>Namek FactionCode=[Z2FactionCode]
<br>Namek ReserveResources=[Z2ReserveResources]
<br>Namek ReserveMana=[Z2ReserveMana]
<br>
<br>
<br>Vegeta Tax=[Z3Tax]<br>
<br>Vegeta ControllingFaction=[Z3ControllingFaction]
<br>Vegeta ControllingRuler=[Z3ControllingRuler]
<br>Vegeta FactionCode=[Z3FactionCode]
<br>Vegeta ReserveResources=[Z3ReserveResources]
<br>Vegeta ReserveMana=[Z3ReserveMana]
<br>
<br>
<br>Icer Tax=[Z4Tax]
<br>Icer ControllingFaction=[Z4ControllingFaction]
<br>Icer ControllingRuler=[Z4ControllingRuler]
<br>Icer FactionCode=[Z4FactionCode]
<br>Icer ReserveResources=[Z4ReserveResources]
<br>Icer ReserveMana=[Z4ReserveMana]
<br>
<br>
<br>Arconia Tax=[Z5Tax]
<br>Arconia ControllingFaction=[Z5ControllingFaction]
<br>Arconia ControllingRuler=[Z5ControllingRuler]
<br>Arconia FactionCode=[Z5FactionCode]
<br>Arconia ReserveResources=[Z5ReserveResources]
<br>Arconia ReserveMana=[Z5ReserveMana]
<br>
<br>
<br>Dark Planet Tax=[Z6Tax]
<br>Dark Planet ControllingFaction=[Z6ControllingFaction]
<br>Dark Planet ControllingRuler=[Z6ControllingRuler]
<br>Dark Planet FactionCode=[Z6FactionCode]
<br>Dark Planet ReserveResources=[Z6ReserveResources]
<br>Dark Planet ReserveMana=[Z6ReserveMana]
<br>
<br>
<br>Space Station Tax=[Z7Tax]
<br>Space Station ControllingFaction=[Z7ControllingFaction]
<br>Space Station ControllingRuler=[Z7ControllingRuler]
<br>Space Station FactionCode=[Z7FactionCode]
<br>Space Station ReserveResources=[Z7ReserveResources]
<br>Space Station ReserveMana=[Z7ReserveMana]
</div>
</body>
</html>
	...
	"}
			usr<<browse(TXview,"window='Taxes';size=350x250")
...
/*	verb/Grant_Building_Permit(mob/player/M in oview(5))
		set category="Rank"
		if(usr.Confirm("Grant [M] temporary building permission? This will last for 1 year."))
			for(var/mob/player/MM in view(10)) MM.AllOut("[usr] has granted [M] a building permit until year [Year+1].")
			M.BuildingPermit=Year+1*/
	...
	verb/Set_Tax_Rate()
		...
		var/list/CPs=list()
		if(usr.TaxCD >= Year)
			usr <<  "You can't change the taxes until year [usr.TaxCD]"
			return
		if(Z1ControllingRuler==usr.name||usr.FactionApproved(Z1FactionCode,4)) CPs+="Earth"
		if(Z2ControllingRuler==usr.name||usr.FactionApproved(Z2FactionCode,4)) CPs+="Namek"
		if(Z3ControllingRuler==usr.name||usr.FactionApproved(Z3FactionCode,4)) CPs+="Vegeta"
		...
		var/CP=input("Which Control Point?") in CPs
		if(CP=="Cancel") return
		var/TaxRate = input("What percentage of mana and resources gains from this planet do you want to tax? (0 - 90%)") as num
		if(TaxRate < 0) TaxRate=0
		if(TaxRate > 90) TaxRate=90
		...
		switch(CP)
			if("Earth")													//If the option is Earth
				Z1FactionTax=FT											//The faction tax = to our input
				Z1Tax=TaxRate											//The tax from this Z plane = to TaxRate
				if(Z2ControllingRuler==usr.name)						//If the Z2 CP ruler = to the name of the player how is changing the Taxes from earth
					Z2FactionTax=FT
					Z2Tax=TaxRate
				if(Z3ControllingRuler==usr.name)
					Z3FactionTax=FT
					Z3Tax=TaxRate
				if(Z4ControllingRuler==usr.name)
					Z4FactionTax=FT
					Z4Tax=TaxRate
				if(Z5ControllingRuler==usr.name)
					Z5FactionTax=FT
					Z5Tax=TaxRate
				if(Z6ControllingRuler==usr.name)
					Z6FactionTax=FT
					Z6Tax=TaxRate
				if(Z7ControllingRuler==usr.name)
					Z7FactionTax=FT
					Z7Tax=TaxRate
			if("Namek")
				Z2FactionTax=FT
				Z2Tax=TaxRate
				if(Z1ControllingRuler==usr.name)						//If the Z1 CP ruler = to the name of the player how is changing the Taxes from Namek
					Z1FactionTax=FT
					Z1Tax=TaxRate
				if(Z3ControllingRuler==usr.name)
					Z3FactionTax=FT
			...
			if("Vegeta")
				Z3FactionTax=FT
				Z3Tax=TaxRate
				if(Z2ControllingRuler==usr.name)						//If the Z3 CP ruler = to the name of the player how is changing the Taxes from Vegeta
					Z2FactionTax=FT
					Z2Tax=TaxRate
				if(Z1ControllingRuler==usr.name)
					Z1FactionTax=FT
					Z1Tax=TaxRate
				if(Z4ControllingRuler==usr.name)
					Z4FactionTax=FT
			...
			if("Icer")
				Z4FactionTax=FT
				Z4Tax=TaxRate
				if(Z2ControllingRuler==usr.name)						//If the Z4 CP ruler = to the name of the player how is changing the Taxes from Icer
					Z2FactionTax=FT
					Z2Tax=TaxRate
				if(Z3ControllingRuler==usr.name)
					Z3FactionTax=FT
			...
			if("Arconia")
				Z5FactionTax=FT
				Z5Tax=TaxRate
				if(Z2ControllingRuler==usr.name)						//If the Z2 CP ruler = to the name of the player how is changing the Taxes from Arconia
					Z2FactionTax=FT
					Z2Tax=TaxRate
				if(Z3ControllingRuler==usr.name)
					Z3FactionTax=FT
			...
			if("Dark Planet")
				Z6FactionTax=FT
				Z6Tax=TaxRate
				if(Z2ControllingRuler==usr.name)						//If the Z2 CP ruler = to the name of the player how is changing the Taxes from Dark Planet
					Z2FactionTax=FT
					Z2Tax=TaxRate
				if(Z3ControllingRuler==usr.name)
					Z3FactionTax=FT
			...
			if("Space Station")
				Z7FactionTax=FT
				Z7Tax=TaxRate
				if(Z2ControllingRuler==usr.name)						//If the Z2 CP ruler = to the name of the player how is changing the Taxes from Space Station
					Z2FactionTax=FT
					Z2Tax=TaxRate
				if(Z3ControllingRuler==usr.name)
					Z3FactionTax=FT
