...
//					stat("+Energy:","[round(MaxKi)] ([KiMult] x) [StatCapCheck(BaseMaxKi/KiMod)<0.4?"<font color=#FF99FF>Max ([round(BaseMaxKi/KiMod)])</font>":StatCapCheck(BaseMaxKi/KiMod)<1?"<font color=#CCCC99>Overcapped ([round(BaseMaxKi/KiMod)])</font>":StatCapCheck(BaseMaxKi/KiMod)<1.5? "<font color=green>Capped ([round(BaseMaxKi/KiMod)])</font>":"<font color=yellow>Uncapped ([round(BaseMaxKi/KiMod)])</font>"]")
					stat("Energy:","[round(MaxKi)] ([KiMult] x)")
					stat("Strength:","[round(StrMod*StrMult,0.01)] ([StrMult] x)")
					stat("Force:","[round(StrMod*PowMult,0.01)] ([PowMult] x)")
					stat("Endurance:","[round(EndMod*EndMult,0.01)] ([EndMult] x)")
					stat("Speed:","[round(SpdMod*SpdMult,0.01)] ([SpdMult] x)")
					stat("Offense:","[round(OffMod*OffMult,0.01)] ([OffMult] x)")
					stat("Defense:","[round(DefMod*DefMult,0.01)] ([DefMult] x)")
					...
					stat("--------------------------")
//				stat("CD between attacks","[round(Refire/100,0.01)]")
				...
				stat("--------------------------")
				stat("Fishing", "\[[Fishing_Level] : [Fishing_Next]/[Fishing_XP]]")
				stat("Cooking", "\[[Cooking_Level] : [Cooking_Next]/[Cooking_XP]]")
				stat("Smithing", "\[[Smithing_Level] : [Smithing_Next]/[Smithing_XP]]")
				stat("Mining", "\[[Mining_Level] : [Mining_Next]/[Mining_XP]]")
				stat("--------------------------")
...
//							var/PowerComparationA= (usr.BP*(usr.StrMod*usr.StrMult)*(usr.EndMod*usr.EndMult)*(usr.SpdMod*usr.SpdMult)*(usr.StrMod*usr.PowMult)*(usr.OffMod*usr.OffMult)*(usr.DefMod*usr.DefMult))
//							var/PowerComparationB= (Target.BP*(Target.StrMod*Target.StrMult)*(Target.EndMod*Target.EndMult)*(Target.SpdMod*Target.SpdMult)*(Target.StrMod*Target.PowMult)*(Target.OffMod*Target.OffMult)*(Target.DefMod*Target.DefMult))
//							var/PowerComparationEnd= round((PowerComparationA*100)/PowerComparationB)
							...
							if(Target.KOd==0)
//								stat("Aura","[PowerComparationEnd] % your Aura")
								stat("Battle Power","[PowDisplay]")
